...
// http://127.0.0.1:8080/chatGPT/chat
  ...
  const updatePopupToolbarLocation = (node) => {
    ...
    const toolbarPadding = 10; // Padding to keep the toolbar away from the edges
    ...
    let newTop = selectionRect.top + 80;
    let newLeft =
      selectionRect.left +
      editorRect.left +
    ...
    if (newTop + PopupToolbarRect.height > viewportHeight) {
      newTop = viewportHeight - PopupToolbarRect.height - toolbarPadding;
    } else if (newTop < 0) {
      newTop = toolbarPadding;
    }
  
    // Adjust for horizontal boundaries
    if (newLeft + PopupToolbarRect.width > viewportWidth) {
      newLeft = viewportWidth - PopupToolbarRect.width - toolbarPadding;
    } else if (newLeft < 0) {
      newLeft = toolbarPadding;
    }
  
    // Set the PopupToolbar Position
    setPopupToolbarTop(newTop + "px");
    setPopupToolbarLeft(newLeft + "px");
  };
